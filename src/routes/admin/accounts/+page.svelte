<script>
  ...
  const filteredProfiles = $derived(
    data.profiles.filter(p => p.username?.toLowerCase().includes(searchQuery.toLowerCase()) || p.email?.toLowerCase().includes(searchQuery.toLowerCase()))
  );

  const totalPages = $derived(Math.ceil(filteredProfiles.length / itemsPerPage) || 1);
          ...
          {#if paginatedProfiles.length === 0}
            <tr>
              <td colspan="4" class="text-center py-5">
                <i class="bi bi-people text-muted opacity-25 d-block mb-3" style="font-size: 3rem;"></i>
                <span class="fsc-3 text-muted">No accounts found.</span>
              </td>
            </tr>
          {:else}
            {#each paginatedProfiles as profile}
              <tr class="border-bottom border-accent-200" style="transition: all 0.2s;">
                <td class="py-4 px-4">
                  <div class="d-flex align-items-center gap-3">
                    ...
                    <span class="fsc-3 fw-bold text-dark">{profile.username}</span>
                  </div>
                </td>
                <td class="py-4 px-4">
                ...
                <td class="py-4 px-4 text-center">
                  {#if profile.role === 'admin'}
                    <span class="badge bg-accent-500 text-white rounded-pill px-3 py-1 fsc-2 fw-bold">Admin</span>
                  {:else}
                    <form action="?/updateRole" method="POST" use:enhance class="position-relative mx-auto" style="min-width: 120px; width: fit-content;">
                      <input type="hidden" name="id" value={profile.id} />
                      <button 
                        type="button"
                        onclick={() => openDropdownId = openDropdownId === profile.id ? null : profile.id}
                      ...
                      >
                        {profile.role === 'cashier' ? 'Cashier' : 'User'} <i class="bi bi-chevron-down"></i>
                      </button>

                      {#if openDropdownId === profile.id}
                        <div class="position-absolute bg-white rounded-3 b-shadow-s border border-accent-500 mt-2 py-2 d-flex flex-column animation-slideUp w-100" style="top: 100%; left: 0; z-index: 1040;">
                          <button name="role" value="user" type="submit" class="curr-item w-100 text-start px-4 py-2 border-0 d-flex align-items-center rounded-0 {profile.role === 'user' ? 'fw-black text-accent-500' : 'fw-medium text-dark'}" onclick={() => openDropdownId = null}>User</button>
                          <button name="role" value="cashier" type="submit" class="curr-item w-100 text-start px-4 py-2 border-0 d-flex align-items-center rounded-0 {profile.role === 'cashier' ? 'fw-black text-accent-500' : 'fw-medium text-dark'}" onclick={() => openDropdownId = null}>Cashier</button>
                        </div>
                      {/if}
                    </form>
                  {/if}
                ...
                <td class="py-4 px-4 text-end">
                  {#if profile.role !== 'admin'}
                    <form action="?/delete" method="POST" use:enhance onsubmit={(e) => !confirm('Are you sure you want to delete this account?') && e.preventDefault()}>
                      <input type="hidden" name="id" value={profile.id} />
                      <button class="btn hover-button-danger rounded-circle p-2" title="Delete Account" aria-label="Delete {profile.username}'s account">
...
{#if isCreateModalOpen}
  ...
  <div class="pop-up d-flex align-items-center justify-content-center animation-fadeIn" onclick={() => isCreateModalOpen = false}>
    <div class="bg-white p-6 p-md-8 rounded-4 b-shadow-s border border-accent-200 d-flex flex-column animation-slideUp" style="max-width: 500px; width: 90%;" onclick={(e) => e.stopPropagation()}>
      <div class="d-flex justify-content-between align-items-center mb-6">
        <h3 class="fsc-4 fw-black text-dark m-0">Create Account</h3>
        <button onclick={() => isCreateModalOpen = false} class="btn hover-button rounded-circle p-2 d-flex justify-content-center align-items-center" style="width: 40px; height: 40px;">
      ...
      <form action="?/create" method="POST" use:enhance={() => { isSubmitting = true; return async ({ update }) => { await update(); isSubmitting = false; isCreateModalOpen = false; }; }} class="d-flex flex-column gap-4">
        ...
        <div class="d-flex flex-column gap-2">
          ...
          <input type="password" id="password" name="password" required class="w-100 py-3 px-4 rounded-3 border border-2 border-accent-200 outline-0 fsc-3 bg-secondary transition-all" onfocus={(e) => (e.target.style.borderColor = "var(--color-accent-500)")} onblur={(e) => (e.target.style.borderColor = "")} placeholder="Min. 6 characters" />
        </div>

        <div class="d-flex flex-column gap-2">
          <label for="username" class="fsc-3 fw-bold text-dark">Username</label>
          <input type="text" id="username" name="username" required class="w-100 py-3 px-4 rounded-3 border border-2 border-accent-200 outline-0 fsc-3 bg-secondary transition-all" onfocus={(e) => (e.target.style.borderColor = "var(--color-accent-500)")} onblur={(e) => (e.target.style.borderColor = "")} placeholder="Display name" />
        </div>
        ...
        <div class="d-flex flex-column gap-2 position-relative">
          ...
          >
            {createRole === 'cashier' ? 'Cashier' : 'User'} <i class="bi bi-chevron-down"></i>
          </button>

          {#if createRoleMenuOpen}
            <div class="position-absolute bg-white rounded-3 b-shadow-s border border-accent-500 mt-2 py-2 d-flex flex-column animation-slideUp w-100" style="top: 100%; left: 0; z-index: 1040;">
              <button type="button" onclick={() => { createRole = 'cashier'; createRoleMenuOpen = false; }} class="curr-item w-100 text-start px-4 py-2 border-0 d-flex align-items-center rounded-0 {createRole === 'cashier' ? 'fw-black text-accent-500' : 'fw-medium text-dark'}">Cashier</button>
              <button type="button" onclick={() => { createRole = 'user'; createRoleMenuOpen = false; }} class="curr-item w-100 text-start px-4 py-2 border-0 d-flex align-items-center rounded-0 {createRole === 'user' ? 'fw-black text-accent-500' : 'fw-medium text-dark'}">User</button>
            </div>
          {/if}
        </div>
