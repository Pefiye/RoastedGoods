...
<div class="d-flex flex-column gap-4 animation-pageIn">
  <div class="d-flex justify-content-between align-items-center">
    <div class="d-flex align-items-center gap-3">
      <a
        href="/admin/products"
        class="btn p-2 rounded-circle hover-button shadow-sm d-flex justify-content-center align-items-center flex-shrink-0"
    ...
    {#if !isNew && !isLoading && !error}
      ...
      >
        <button
          type="submit"
          class="px-5 py-2 fsc-3 fw-bold rounded-pill border border-2 border-danger text-nowrap hover-button-danger transition-all d-flex align-items-center gap-2 bg-white"
        >
  ...
  {:else}
    ...
    use:enhance={async ({ formData, cancel }) => {
      if (pendingImageBlob) {
        ...
        const webpFile = new File([pendingImageBlob], finalName, {
          type: "image/webp",
        });
  ...
  >
    <div class="col-12 col-xl-8">
      <div
        class="bg-white p-4 p-md-5 rounded-4 b-shadow-s border border-accent-200 h-100 d-flex flex-column gap-4"
      >
        <div class="d-flex flex-column gap-2">
          <label for="name" class="fsc-3 fw-bold text-accent-500"
            >Drink Name</label
          >
          <input
            type="text"
        ...
        <div class="d-flex flex-column gap-2">
          <label for="description" class="fsc-3 fw-bold text-accent-500"
            >Description</label
          >
          <textarea
            id="description"
            ...
            rows="4"
            required
            class="w-100 py-3 px-4 rounded-3 border border-2 border-accent-200 outline-0 fsc-3 bg-secondary transition-all resize-0"
            onfocus={(e) =>
              (e.target.style.borderColor = "var(--color-accent-500)")}
            onblur={(e) => (e.target.style.borderColor = "")}
        ...
        <div class="d-flex flex-column gap-2">
          <label class="fsc-3 fw-bold text-accent-500">Category</label>
          ...
          <div class="position-relative">
            <button
              ...
              class="w-100 px-4 py-3 fsc-3 fw-bold rounded-pill border border-2 border-accent-500 hover-button d-flex align-items-center justify-content-between gap-3"
            >
              {category === "coffee" ? "Coffee" : "Non-Coffee"}
            ...
            {#if categoryMenuOpen}
              ...
              >
                <button
                  ...
                  class="curr-item w-100 text-start px-4 py-2 border-0 d-flex align-items-center rounded-0 {category ===
                  'coffee'
        ...
        <div
          class="border-top border-accent-200 mt-2 pt-4 d-flex flex-column gap-4"
        >
          <h3 class="fsc-4 fw-bold text-accent-500 m-0">Pricing (IDR)</h3>
          ...
          <div class="row g-3">
            <div class="col-12 col-md-4">
              <div class="d-flex flex-column gap-2">
                <label
                  for="tall_price"
                  class="fsc-2 fw-bold text-muted text-uppercase">Tall</label
                >
                <div class="position-relative">
                  <span
                    class="position-absolute top-50 start-0 translate-middle-y ms-3 fsc-3 fw-bold text-muted"
                    >Rp</span
                  >
                  <input
                    type="number"
                    id="tall_price"
                    name="tall_price"
                    bind:value={tallPrice}
                    required
                    min="0"
                    class="w-100 py-3 pe-3 rounded-3 border border-2 border-accent-200 outline-0 fsc-3 bg-secondary"
            ...
            <div class="col-12 col-md-4">
              <div class="d-flex flex-column gap-2">
                <label
                  for="grande_price"
                  class="fsc-2 fw-bold text-muted text-uppercase">Grande</label
                >
                <div class="position-relative">
                  <span
                    class="position-absolute top-50 start-0 translate-middle-y ms-3 fsc-3 fw-bold text-muted"
            ...
            <div class="col-12 col-md-4">
              <div class="d-flex flex-column gap-2">
                <label
                  for="venti_price"
                  class="fsc-2 fw-bold text-muted text-uppercase">Venti</label
                >
                <div class="position-relative">
                  <span
                    class="position-absolute top-50 start-0 translate-middle-y ms-3 fsc-3 fw-bold text-muted"
    ...
    <div class="col-12 col-xl-4">
      <div
        class="bg-white p-4 p-md-5 rounded-4 b-shadow-s border border-accent-200 d-flex flex-column gap-4 sticky-top"
        style="top: 2rem;"
      >
        <h3 class="fsc-4 fw-bold text-accent-500 m-0">Image</h3>

        <input type="hidden" name="image_url" value={imagePreview} required />

        <label
          class="ratio ratio-1 bg-secondary rounded-4 border border-2 border-accent-200 border-dashed cursor-pointer overflow-hidden transition-all"
          for="image_upload"
        >
          <div
            class="d-flex flex-column justify-content-center align-items-center w-100 h-100 position-absolute top-0 start-0"
          >
            {#if isUploading}
              <div
                class="position-absolute top-0 start-0 w-100 h-100 bg-white d-flex flex-column justify-content-center align-items-center z-index-2 opacity-75"
              >
            ...
            {#if imagePreview}
              ...
              />
              <div
                class="position-absolute top-0 start-0 w-100 h-100 bg-dark bg-opacity-50 text-white text-center fsc-3 fw-bold d-flex justify-content-center align-items-center"
            ...
            {:else}
              <div
                class="d-flex flex-column align-items-center gap-2 text-muted"
              >
        ...
        <button
          type="submit"
          class="w-100 py-3 px-5 fsc-3 fw-bold rounded-pill border border-2 border-accent-500 text-nowrap hover-button-alt transition-all d-flex justify-content-center align-items-center gap-2"
