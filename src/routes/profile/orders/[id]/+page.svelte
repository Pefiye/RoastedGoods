  ...
  <div class="w-100">
    <div class="d-flex align-items-center mb-8 gap-4">
      ...
      {#if isLoading}
        <div class="placeholder-glow">
          <span class="placeholder col-4 rounded" style="width: 150px;"></span>
        </div>
      {:else}
    ...
    {#if isLoading}
      <div class="col-12 mb-6 placeholder-glow">
        ...
        >
          <span class="placeholder col-3 rounded mb-4" style="height: 24px;"
          ></span>
          <span
            class="placeholder col-2 rounded-pill mb-3"
            style="height: 38px;"
          ></span>
          <span class="placeholder col-2 rounded-3" style="height: 24px;"
          ></span>
        </div>
      </div>
      <div
        class="bg-white p-6 p-md-8 rounded-4 b-shadow-s border border-accent-200 mb-6 placeholder-glow"
      >
        <div
          class="d-flex flex-column mb-5 pb-5 border-bottom border-accent-200 gap-2"
        >
          <span class="placeholder col-2 rounded"></span>
          <span class="placeholder col-3 rounded"></span>
        </div>
        <span class="placeholder col-2 rounded mb-5" style="height: 24px;"
        ></span>
        <div class="d-flex flex-column gap-4">
          {#each Array(2) as _}
            ...
            >
              <div
                class="rounded-3 placeholder bg-accent-200 flex-shrink-0"
      ...
      <div
        class="bg-white p-6 p-md-8 rounded-4 b-shadow-s border border-accent-200 placeholder-glow"
      >
    ...
    {:else if error}
      <div
        class="text-center p-5 bg-secondary rounded-4 border border-danger border-dashed"
      >
        <i
          class="bi bi-exclamation-triangle text-danger opacity-50 mb-4 d-block"
    ...
    {:else}
        ...
        >
          ...
          {#if order.status === "preparing"}
            <p class="fsc-2 text-muted mt-4 mb-0">
              Your order is being prepared. We'll call your name when it's
              ready!
            </p>
          {/if}
        </div>
      ...
      >
          ...
          <div class="d-flex flex-column gap-4">
            ...
            <div class="d-flex flex-column flex-sm-row gap-3">
              ...
              <button
                onclick={cancelOrder}
                disabled={retrying || cancelling}
                class="px-6 py-3 fw-bold rounded-pill fsc-2 border border-2 border-danger text-nowrap d-flex align-items-center justify-content-center gap-2 bg-white hover-button-danger"
              >
                {#if cancelling}
        ...
        {#if order.status === "paid"}
          <hr class="border-accent-200 my-5 opacity-25" />
          <div
            class="bg-accent-100 p-4 rounded-3 border border-accent-200 d-flex gap-3 align-items-start"
          >
            <i class="bi bi-info-circle-fill text-accent-500 fsc-3 mt-1"></i>
            <p class="fsc-2 text-accent-500 mb-0 fw-medium">
              Your payment was successful! Please head over to the cashier or
              counter so we can start preparing your order.
            </p>
          </div>
        {/if}
